...
class ObjectDetector:
    ...
    def _patch_head(self) -> None:
        ...
        for module in self.model.model.modules():
            if hasattr(module, "detect"):
                ...
                logger.info(f"ObjectDetector: {type(module).__name__}.detect 패치 완료")
    ...
    def detect(self, frame: np.ndarray) -> List[Dict[str, Any]]:
        ...
        if results and results[0].boxes is not None and len(results[0].boxes):
            logger.info(f"[ObjectDetector] {len(results[0].boxes)}개 탐지: {[(int(c), self.model.names[int(c)]) for c in results[0].boxes.cls.cpu().numpy()]}")
